...
export function setLandslide(entry, partyId, pct) {
  ...
  const rivals = PARTIES.filter((p) => p.id !== partyId);
  // Split what's left evenly between the rivals, but keep each at least 3 points
  // behind the leader (matters at 33%); anything left over goes to Others.
  const each = Math.floor(Math.max(0, Math.min((100 - v) / rivals.length, v - 3)) * 2) / 2;
  const shares = Object.fromEntries(PARTIES.map((p) => [p.id, p.id === partyId ? round1(v) : each]));
  return { ...entry, shares };
}

// Gives a non-leading party a share (10-25%) without making it the leader.
// Extra room comes from Others first, then from the biggest other party.
export function setSecondShare(entry, partyId, pct) {
  const v = Math.max(0, Math.min(100, Number.isFinite(pct) ? pct : 0));
  const shares = { ...entry.shares };
  let need = v - (shares[partyId] || 0);
  shares[partyId] = v;
  if (need > 0) {
    const fromOthers = Math.min(need, Math.max(0, 100 - PARTIES.reduce((a, p) => a + (entry.shares[p.id] || 0), 0)));
    need -= fromOthers;
    while (need > 0.01) {
      const donor = PARTIES.filter((p) => p.id !== partyId && (shares[p.id] || 0) > 0).sort(
        (a, b) => shares[b.id] - shares[a.id]
      )[0];
      if (!donor) break;
      const take = Math.min(need, shares[donor.id]);
      shares[donor.id] = round1(shares[donor.id] - take);
      need -= take;
    }
  }
  return { ...entry, shares };
